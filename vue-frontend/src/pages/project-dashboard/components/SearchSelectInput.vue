<template>
    <div class="form-group position-relative">
        <input type="text" class="form-control mb-1 dropdown-toggle" v-model="searchTerm"
            :placeholder="t('searchselectinput.placeholder')" @click="showDropdown = true" @blur="hideDropdown" />
        <ul v-show="showDropdown && filteredOptions.length" class="dropdown-menu w-100" style="display: block">
            ...
            <li v-if="filteredOptions.length === 0" class="dropdown-item disabled">{{ t('general.no-entries') }}</li>
        </ul>
    </div>
</template>
